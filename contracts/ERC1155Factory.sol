...
abstract contract ERC1155Factory is ERC1155 {
    ...
    /// @notice add new item
    function addItem(string memory itemUri, bool isPaused) external {
        Item memory item = Item(itemUri, uint64(items.length), 0, isPaused);
        items.push(item);
    }

    /// @notice edit item.uri
    function editItem_uri(uint256 id, string memory newUri) external {
    ...
    function editItem_isPaused(uint256 id, bool _isPaused) external {
        if (!exists(id)) revert ERC1155_Error404();
        items[id].isPaused = _isPaused;
    }

    /// @notice remove last item if supply != 0
    function removeLastItem() external {
    ...
    function paused(bool _isPaused) external {
